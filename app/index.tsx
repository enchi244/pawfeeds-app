...
const createWebViewHtml = (streamUrl: string) => `
  ...
  <html>
    ...
    <body>
      <img id="stream" src="${streamUrl}" onerror="handleError()" />
      <script>
        function handleError() {
          if (window.ReactNativeWebView && window.ReactNativeWebView.postMessage) {
            window.ReactNativeWebView.postMessage('IMAGE_LOAD_ERROR');
          }
        }
      </script>
    </body>
...
const BowlCard: React.FC<BowlCardProps> = ({ bowlNumber, selectedPet, foodLevel, perMealPortion, onPressFeed, onPressFilter, streamUri }) => {
    ...
    const handleWebViewMessage = (event: WebViewMessageEvent) => {
        ...
        if (message === 'IMAGE_LOAD_ERROR') {
            console.error(`[WebView Bowl ${bowlNumber}] The <img> tag within the HTML failed to load the stream. This often indicates a network or mixed-content issue.`);
        }
    };
    ...
    return (
      <View style={[styles.card, { width: CARD_WIDTH }]}>
        ...
        <View style={styles.videoFeedPlaceholder}>
          {streamUri ? (
            // +++ THE FINAL FIX: Add Android-specific props to force rendering +++
            <WebView
              style={styles.video}
              originWhitelist={['*']}
              ...
              onError={(event) => console.error(`[WebView Bowl ${bowlNumber}] Native Error: ${event.nativeEvent.description}`)}
              onLoad={() => console.log(`[WebView Bowl ${bowlNumber}] HTML container loaded successfully.`)}
              // Allows HTTP content to be loaded in a WebView. CRITICAL for MJPEG streams.
              mixedContentMode="always"
              // Disables hardware acceleration for this WebView, which can resolve rendering glitches with streams on some Android devices.
